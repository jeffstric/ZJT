...
宫格整图含 N 个分镜格子，拆分后单格分辨率 ≈ 整图分辨率/N，因此宫格生图**始终按模型支持的最高分辨率档位提交**（前端 `TaskConfig.getMaxSupportedSize(modelKey)` 从 task-configs 的 `supported_sizes` 中按数字取最大，大小写归一后以大写提交，如 GPT Image 2 → `4K`、加强版 → `4K`）：

- **GPT Image 2**: `image_size=4K`（9:16 实际 2160x3840，9 宫格单格约 720x1280）
- **加强版 (gemini-3-pro)**: `image_size=4K`
- **模型未配置 `supported_sizes`**（列表为空或非 `1K/2K/4K` 档位）：不传该字段，走后端 `/api/image-edit` 默认值 `1K`

> 历史背景：此前仅加强版（`gemini-3-pro-image-preview`）硬编码传 `4K`，其余模型（含 GPT Image 2）不传 `image_size`，后端默认 `1K`，导致 9 宫格单格分辨率过低、画质差（线上工单 40892）。2026-09 起改为按模型最高档通用提交。
...
6. **最高分辨率输出**: 宫格生图按模型支持的最高分辨率档位提交（如 GPT Image 2 / 加强版 → 4K），确保拆分后单格画质；确认弹窗会显示本次使用的分辨率
