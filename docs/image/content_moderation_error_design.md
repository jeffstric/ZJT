...
| 2026-09-03 | **HTTP 4xx/5xx 响应体保留 + GPT Image 2 驱动接入**：`base_video_driver._request` 在 `raise_for_status()` 抛出 HTTPError 时把已解析的响应体挂到 `e.response_body`（此前 4xx 原文直接丢失，审核/额度错误只能显示「服务异常」）；`gpt_image_common_v1_driver` / `gpt_image_duomi_v1_driver` 提交失败的通用异常分支优先解析 `response_body.error`，经 `build_user_error_from_api_error` 产出审核友好文案或真实业务错误（如 `user quota is not enough`），按 USER 错误透传。修复了 sync_mode 链路（`sync_task_executor` 原样透传驱动 error，无需改动）下审核/额度信息被吞的问题。前端 `web/js/image_node.js` 图片编辑 onComplete 补充失败任务原因展示（违规时弹 `ContentViolation.notify`），不再误报「生成成功，但未获取到图片地址」 |
| 2026-09-03 | **轮询中文案与 duomi 上游原因透传**：`process_task_with_retry` 的 RETRY_SCHEDULED 时间线文案区分「任务处理中，安排下次轮询」（RUNNING 轮询返回 False 的正常调度，该机制自 2026-02 存在）与「处理失败，安排重试」（真实失败），消除误报；`gpt_image_duomi_v1_driver.check_status` 上游 `state=error` 时透传 `message` 字段（如「抱歉，任务处理遇到了一点小问题」），不再只返回写死的「图片生成失败」 |
