...
- `3:2` -> `1536x1024` (横版)
- `2:3` -> `1024x1536` (竖版)
- `16:9` -> `1536x864` (横版，真比例自定义尺寸)
- `9:16` -> `864x1536` (竖版，真比例自定义尺寸)

> 注：官方 Image API 的 `size` 支持任意 16 倍数的自定义尺寸（宽高比须在 1:3~3:1 之间），
> 因此 1K 档位的 `16:9` / `9:16` 映射为真比例像素，不再复用 `1536x1024` / `1024x1536`（实为 3:2 / 2:3）。
...
多米 API 的 `size` 参数支持显式 `宽x高` 像素（宽高需被 16 整除，单边 [16, 3840]，像素预算 655,360 ≤ 宽x高 ≤ 8,294,400）。驱动按 `image_size + ratio` 映射为显式像素，`image_size` 大小写不敏感，未知档位回退 1k、未知比例回退 1:1：

| 档位 | 1:1 | 3:2 | 2:3 | 16:9 | 9:16 |
|------|-----|-----|-----|------|------|
| 1k | 1024x1024 | 1536x1024 | 1024x1536 | 1280x720 | 720x1280 |
| 2k | 2048x2048 | 2048x1360 | 1360x2048 | 2048x1152 | 1152x2048 |
| 4k | 2880x2880 | 3520x2352 | 2352x3520 | 3840x2160 | 2160x3840 |

注意：16:9 / 9:16 在各档位均映射为**真比例**像素，不再兼容映射到 3:2 / 2:3。
...
- `2:3` - 竖版
- `3:2` - 横版
- `16:9` - 真比例横版（1K 档位映射为 `1536x864`）
- `9:16` - 真比例竖版（1K 档位映射为 `864x1536`）
