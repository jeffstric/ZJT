...
交互约定（直填生图与直连视频一致）：点击发送后聊天区即时插入**用户气泡**（`[直填生图] <提示词>` / `[视频生成] <提示词>`，对口型分镜一键提交为固定文案「[视频生成] 一键提交对口型视频生成」）与**提交状态气泡**（「生图/视频生成任务已提交，右侧候选区将显示生成进度」，提交失败则留痕「提交失败：<原因>」，alert 与「内容违规提醒」弹框保留不变）；分镜已有任务在处理时再次点击发送，以 toast 提示「当前分镜有任务正在处理中，请稍候」（此前为静默返回，用户误以为未提交，2026-09 修复）。注意直连模式的气泡为前端本地内存消息（与智能体模式不同，不落聊天历史接口），切换分镜后不保留。左下角提示行**提交前常驻显示预估**（`estimateScenePower`，随模式/所选模型/时长档变化，如「⚡ 预计生图消耗 2 算力」），提交成功后切换为实际消耗（`state.lastPowerSpend`，如「⚡ 本次生图消耗 2 算力」），同时异步刷新右上角算力余额。模式切换、分镜切换、模型/时长选择变化会清除实际值、回到预估显示。预估口径：**图片/编辑模型为固定单价**（直填生图提交走 `getSelectedImageTaskId(true)` 图生图/编辑槽位，预估同槽位）；**视频一律经后端单镜估价接口** `POST /scene/{id}/estimate-video-power`（纯只读，`storyboard:view`）——口径与扣费完全一致：时长档位基价 × 分辨率/图模式修饰符倍率（DB 可热更新）向上取整，即 `get_computing_power_for_task(context={resolution, image_mode})`；前端不复刻倍率表。对口型分镜由接口内数字人服务端规划（billable_duration）精确计价。前端以「分镜+模型+时长档+分辨率+图模式」为缓存键，参数变化自动重拉；估价未返回时短暂显示「正在预估消耗…」。弹窗内 select、「性价比/效果」档位按钮、分辨率切换三条换模型路径都会即时重算预估。直连 generate-video 的预扣也已对齐同口径（带 resolution/image_mode context，此前漏乘分辨率倍率）。生图路径响应顶层 `computing_power` 由 `_finalize_submission` 从 `submission.computing_power_required/total` 提升而来（与 generate-video 路径字段对齐）；提交/生成失败仍走 alert 与「内容违规提醒」弹框。
