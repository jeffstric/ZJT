...
交互约定（直填生图与直连视频一致）：点击发送**不弹任何确认/提示 alert**，直接提交；左下角提示行**提交前常驻显示预估**（`estimateScenePower`，随模式/所选模型/时长档变化，如「⚡ 预计生图消耗 2 算力」），提交成功后切换为实际消耗（`state.lastPowerSpend`，如「⚡ 本次生图消耗 2 算力」），同时异步刷新右上角算力余额。模式切换、分镜切换、模型/时长选择变化会清除实际值、回到预估显示。预估口径：图片/编辑模型为固定单价；视频按时长计费时按当前 duration 匹配 `computing_power_map` 档位（`/models` 接口返回的完整档位表，取值与后端 `get_computing_power(duration=...)` 一致，无匹配取首档）；对口型分镜按分镜时长向上取整到 5s 档预估数字人链路。生图路径响应顶层 `computing_power` 由 `_finalize_submission` 从 `submission.computing_power_required/total` 提升而来（与 generate-video 路径字段对齐）；提交/生成失败仍走 alert 与「内容违规提醒」弹框。
