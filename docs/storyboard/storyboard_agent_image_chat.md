...
交互约定（直填生图与直连视频一致，2026-09 修订，不走聊天区气泡）：点击发送后**底部 toast 就近确认**「生图/视频生成任务已提交，右侧候选区生成中」（`sb-toast` 固定底部居中，紧邻左下角输入区），同时右侧候选区**立即插入「生成中/视频生成中」乐观占位卡**（`events.js` `insertOptimisticGeneratingCandidate` 向 `state.sceneCandidates[sceneId]` 顶部插入空 url 临时条目并刷新 `Region.CANDIDATES`，渲染层对空 url 天然显示占位卡，非数字临时 id 下不可点选/无删除按钮）；提交响应自带真实资产 id（生图 `result.asset_ids[0]`、生视频 `result.asset_id`），`adoptOptimisticCandidate` 原地替换临时 id 后由既有 `loadSceneCandidates` + 轮询 `upsertGeneratingCandidate`（按 String(id) 去重）无缝接管，不会出现重复占位卡；提交失败 `removeOptimisticCandidate` 移除乐观卡并保留原有 alert 与「内容违规提醒」弹框。分镜已有任务在处理时再次点击发送，以 toast 提示「当前分镜有任务正在处理中，请稍候」（此前为静默返回）。左下角提示行**提交前常驻显示预估**（`estimateScenePower`，随模式/所选模型/时长档变化，如「⚡ 预计生图消耗 2 算力」），提交成功后切换为实际消耗（`state.lastPowerSpend`，如「⚡ 本次生图消耗 2 算力」），同时异步刷新右上角算力余额。模式切换、分镜切换、模型/时长选择变化会清除实际值、回到预估显示。预估口径：**图片/编辑模型为固定单价**（直填生图提交走 `getSelectedImageTaskId(true)` 图生图/编辑槽位，预估同槽位）；**视频一律经后端单镜估价接口** `POST /scene/{id}/estimate-video-power`（纯只读，`storyboard:view`）——口径与扣费完全一致：时长档位基价 × 分辨率/图模式修饰符倍率（DB 可热更新）向上取整，即 `get_computing_power_for_task(context={resolution, image_mode})`；前端不复刻倍率表。对口型分镜由接口内数字人服务端规划（billable_duration）精确计价。前端以「分镜+模型+时长档+分辨率+图模式」为缓存键，参数变化自动重拉；估价未返回时短暂显示「正在预估消耗…」。弹窗内 select、「性价比/效果」档位按钮、分辨率切换三条换模型路径都会即时重算预估。直连 generate-video 的预扣也已对齐同口径（带 resolution/image_mode context，此前漏乘分辨率倍率）。生图路径响应顶层 `computing_power` 由 `_finalize_submission` 从 `submission.computing_power_required/total` 提升而来（与 generate-video 路径字段对齐）；提交/生成失败仍走 alert 与「内容违规提醒」弹框。
