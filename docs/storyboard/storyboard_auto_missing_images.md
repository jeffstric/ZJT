...
- In quality mode, QS performs one compact two-dimensional review after prompt rewriting when a shot provides fixed `point_id`, `physical_position.side`, raw `screen_position`, and a complete `screen_axis_mapping`. The reviewer rejects only an explicit left/right reversal, then sends the issue and the previous QS output back to the same model for one targeted repair. Mirrored axis mappings are valid; center, center-left/center-right, depth, occlusion, missing facts, and ambiguous descriptions do not trigger review failure.
- Quality-grid `style` and `composition_preference` are emitted once at the grid JSON root as `global_visual_guidance.image_style` and `global_visual_guidance.composition_preference`; they are no longer repeated in every `shots[].prompt_text`. Grid-canvas formatting rules such as no text, captions, numbers, or blank caption bars live separately in `grid_output_constraints`, so they cannot be confused with visual style guidance. The root visual object also carries one `application_rule`: it applies to every non-placeholder cell, while explicit per-cell camera, framing, subject placement, and composition constraints take precedence. The commercial policy is implemented by `enterprise.services.storyboard_quality_sequence.QualityStoryboardPromptPolicy` and loaded through the shared delayed-import facade. The optional LLM rewriter receives the same root guidance as separate input, is instructed not to copy it into cell prompts, and the policy removes exact duplicate visual blocks before final submission. Multi-line textarea values are normalized from CRLF/CR to LF and matched as complete line-aligned blocks, preserving unrelated body lines while keeping repeated helper calls idempotent. `grid_prompt_group_context` stores the guidance once at group scope for diagnostics and continuity. Empty visual values are omitted.
...
- **生图模型快照透传与对账（2026-08-15 线上事故修复）**：quality 首帧宫格链路此前未透传用户在拆分界面选择的生图模型，`submit_grid_image_task` 内部回退到当时的默认模型（nano-banana-Pro，`task_id=7`），导致用户选择 GPT Image 2（task_id=26）实际按 nano-banana-Pro 生图。现行无偏好兜底已改为 GPT Image 2（`DEFAULT_TEXT_TO_IMAGE_TASK_ID=26`）。规则：
  - `StoryboardFirstFrameGridService._submit_chunk` 必须从**批任务创建时锁定的快照**解析模型（`extra_json.generation_snapshots['image.image_edit'].task_id`，回退 `extra_json.task_type`），通过 `submit_grid_image_task(task_type=...)` 显式透传。禁止在提交时实时读用户偏好：quality 跨幕串行执行时间长，中途偏好变化会导致前后分镜模型/画风不一致。
  - `submit_grid_image_task` 收到显式 `task_type` 后原样使用；所选模型不属于 IMAGE_EDIT 类别时整批 item 明确失败（报错带模型名，禁止静默换模型）。未显式指定时才回退偏好/默认解析，并打 `[GRID] ... 未显式指定生图模型` warning 让隐式解析点在日志中现形。
  - 运行时对账防线：宫格提交后校验返回值 `model_task_id` 与快照一致（`[quality-grid] 宫格提交模型与批任务快照不一致`）；非 quality 批任务提交后同样校验 `submission.model_task_id`（`[batch-model-audit] 批任务提交模型与快照不一致`）。不一致仅打 error 供日志/Sentry 告警，不改变已提交结果。新增链路接入生图时必须透传批任务快照模型，防回归测试见 `tests/services/test_storyboard_first_frame_grid_service.py::test_submit_chunk_passes_locked_task_type_from_batch_snapshot` 与 `tests/script_writer_core/test_submit_grid_image_task_model_passthrough.py`。
