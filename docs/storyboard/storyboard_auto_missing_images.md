...
- Every real quality-grid cell receives the storyboard-level `style` and `composition_preference` as authoritative prompt suffixes (`图片风格：...` / `构图倾向：...`). The commercial policy is implemented by `enterprise.services.storyboard_quality_sequence.QualityStoryboardPromptPolicy` and loaded through the shared delayed-import facade. It injects the suffixes before optional LLM rewriting and restores them before `submit_grid_image_task`, so an LLM response cannot accidentally remove the global visual style. The core grid service only exposes generic before/after rewrite hooks and contains no commercial prompt rule text. Empty values are skipped and identical suffixes are de-duplicated.
