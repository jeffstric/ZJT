...
- Quality-grid `style` and `composition_preference` are emitted once at the grid JSON root as `global_visual_guidance.image_style` and `global_visual_guidance.composition_preference`; they are no longer repeated in every `shots[].prompt_text`. The root object also carries one `application_rule`: it applies to every non-placeholder cell, while explicit per-cell camera, framing, subject placement, and composition constraints take precedence. The commercial policy is implemented by `enterprise.services.storyboard_quality_sequence.QualityStoryboardPromptPolicy` and loaded through the shared delayed-import facade. The optional LLM rewriter receives the same root guidance as separate input, is instructed not to copy it into cell prompts, and the policy removes exact duplicate visual lines before final submission. `grid_prompt_group_context` stores the guidance once at group scope for diagnostics and continuity. Empty visual values are omitted.
