...
- **生图模型快照透传与对账（2026-08-15 线上事故修复）**：quality 首帧宫格链路此前未透传用户在拆分界面选择的生图模型，`submit_grid_image_task` 内部回退到 legacy 偏好/默认模型（`DEFAULT_TEXT_TO_IMAGE_TASK_ID=7` nano-banana-Pro），导致用户选择 GPT Image 2（task_id=26）实际按 nano-banana-Pro 生图。现行规则：
