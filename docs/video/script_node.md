...
- **均衡模式 (`balanced`)**：质量与效率折中，两版本默认选中值。**社区版**下显示「（推荐）」。
- **速度模式 (`speed`)**：快速拆分，适合草稿试跑。
- **效果模式 (`quality`)**：影院级一致性；**仅商业版**可用。**商业版**下显示「（推荐）」且可选；**社区版**下拉禁用并标「（商业版）」，若强选会 toast 提示并回退。

> 「（推荐）」标签由 `applySequenceModeLabels()` 依据 `isEnterpriseEdition()` 动态渲染：社区版→均衡，商业版→效果。默认选中值始终为 `balanced`。`balanced`/`quality` 两个选项**不携带 `data-i18n`**，文案由 JS 纯管理（避免 i18n DOM 重扫冲掉「推荐」标签），并在 `syncSequenceModeUi()`（含工作流重载）与 `locale-changed`（切换语言）时重算。
