...
智剧通 API 正在逐步下线，卡片上已不再显示「推荐」标签。

弹窗右上角的 **「快速选择」** 会一键选中当前推荐组合：

| 服务商 | 分类 | 说明 |
|--------|------|------|
| DeepSeek | 大模型 | 剧本创作、AI 对话、剧本拆分 |
| 多米 (Duomi) | 生图 + 生视频 | 共用同一 Token，覆盖 Nano Banana、Sora2、Kling、Veo3 等 |

也可在左侧手动勾选其他服务商（含智剧通 API、火山引擎、Qwen 等）。
...
2. **选择服务商（可选）**
   - 点击右上角「快速选择」自动选中 DeepSeek + 多米
   - 或在左侧按分类手动勾选服务商（大模型 / 多媒体，多媒体合并生图+生视频+其他；每个分类内推荐供应商优先排在最前）
   - 点击已选中的卡片不会取消选中，而是联动右侧定位到对应配置卡片；取消选中请用右侧卡片右上角的 ✕
   - 若后端已有某服务商的必填配置（如多米 Token），打开弹窗时会自动勾选；共享同一配置键的条目（如 `volcengine.api_key` 对应的火山大模型与多媒体条目）会同时勾选
...
4. **测试连接（可选）**
   - 对于 Google / Qwen 配置，可点击「测试连接」验证 API Key 有效性

5. **保存配置**
   - 点击「保存所有配置」一次性保存
   - 或点击右侧单个配置卡片上的「保存」按钮
   - 配置立即生效，无需重启服务
   - 保存后若某服务商的密钥（API Key/Token）为空，左侧选中会自动撤销（右侧配置卡片随之移除）；共享同一密钥的条目（如火山大模型/多媒体）按 `baseName` 整组一并撤销，可再次点击左侧卡片选中并重新填写
