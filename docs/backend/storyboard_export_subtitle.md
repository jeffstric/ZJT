...
### smart 长句二级细分（2026-09-07 补充）
- 实测问题：SenseVoice 可能把句末感叹号转写成逗号（如"不可行！"→"不可行，"），
  句末标点丢失导致 ASR 句粒度过粗——一句 30+ 字在竖屏（每行 16 字）折 3 行
  同屏 6 秒+，观感回到 block 整段堆积（分镜 5 / 对白 204 实例）；
- 修复：smart 分支中单条 ASR 句折行超过 `SMART_CUE_MAX_LINES=2` 行时，
  按可断标点（句末+句中）切成原子片段、贪心合并到 ≤2 行的多条 cue，
  时长按去标点字符占比内插（`split_long_seg_by_inner_punct`）；
- 标点太稀（存在单片段自身超 2 行）时细分失败，沿用原单 cue/多页分页逻辑；
- block 模式行为不变；`MAX_LINES=3` 仍为硬烧版式的单页行数上限。
