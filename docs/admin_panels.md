...
| 操作 | 说明 |
|------|------|
| 查看详情 | 查看用户完整信息（ID、手机号、角色、状态、算力、邀请码、渠道推广等级、注册时间） |
| 调整算力 | 增加或扣减用户算力（需填写原因） |
| 审批登录 | 对状态为"待审核"的用户进行审批通过 |
| 开佣金/关佣金 | 开启或关闭该用户的渠道现金佣金（非社区版）。默认用户邀请链接只发算力；开启后其邀请的充值才会产生佣金 |
...
- **密钥毛玻璃遮罩**：右侧配置卡片中的密钥字段（api_key/token）默认覆盖一层毛玻璃遮罩，密钥不可见；点击输入框右侧的眼睛按钮后移除遮罩显示完整密钥，再次点击眼睛可重新遮住。空字段不显示遮罩，可直接输入。共享同一密钥的条目（按 `baseName` 分组，如火山大模型/多媒体）眼睛状态同步。每次打开弹窗遮罩恢复默认遮住状态
...
| `time_period` | 计费时段：`normal`=不分峰谷（默认，向后兼容）、`peak`=高峰、`off_peak`=空闲。详见 5.6 峰谷计费 |
...
- **官方 DeepSeek**：迁移 `20260813_vendor_model_time_period` 已为其 `flash/pro` 初始化峰谷两档；`flash-vision-exp` 由迁移 `no_123_20260901_ds_vision_peak_valley` 修正为峰谷两档（与 flash 同价）；其他中转商（zjt_api / 火山）是否峰谷需在界面按实际计费规则单独配置
- **时段说明展示**：模型管理页顶部「大模型分段计费说明」与「AI 生成方案」目标计费模式下拉旁均标注峰谷具体时段（北京时间周一至周五 9:00-12:00、14:00-18:00 高峰，其余含周末全天空闲）；计费档位弹窗时段下拉下方 hint 同步说明

> DeepSeek 官方峰谷价（元/百万 token）：flash 高峰 3/9/0.10、空闲 1.5/4.5/0.05；pro 高峰 9/27/0.30、空闲 4.5/13.5/0.15；flash-vision-exp 与 flash 同价（高峰 3/9/0.10、空闲 1.5/4.5/0.05）。
