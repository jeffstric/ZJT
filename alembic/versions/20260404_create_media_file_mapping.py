...
def upgrade() -> None:
    ...
    op.create_table('media_file_mapping',
        ...
        sa.Column('local_path', sa.Text(), nullable=False, comment='本地文件相对路径'),
        ...
        sa.Column('entity_type', sa.Integer(), nullable=False, comment='实体类型（枚举值）'),
        sa.Column('source_id', sa.Integer(), nullable=True, comment='实体ID'),
        sa.Column('media_type', sa.String(50), nullable=False, comment='媒体类型（MIME type）'),
...
def downgrade() -> None:
    ...
    op.drop_index('idx_entity', table_name='media_file_mapping')
