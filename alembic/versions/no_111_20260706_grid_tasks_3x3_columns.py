"""grid_image_tasks 新增 3x3 九宫格与 i2i 父图相关列
...
新增列：
  ...
  - parent_reference_image VARCHAR(1000) i2i 父场景参考图 URL（重试时复原）
...
def upgrade() -> None:
    """升级：为 grid_image_tasks 新增九宫格 + i2i 相关列。"""
    conn = op.get_bind()

    # 逐列添加，使用 IF NOT EXISTS 风格的幂等检查（MySQL 8 支持，低版本用 information_schema 兜底）
    columns_to_add = [
        ...
        ("parent_reference_image", "VARCHAR(1000) NULL COMMENT 'i2i 父场景参考图 URL'", "target_entity_ids_json"),
    ]
