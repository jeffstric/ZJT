...
class GridConfig:
    ...
    VALIDATION_MIN_LINE_COVERAGE = 0.75    # 分割线贯穿比例阈值（细线友好采样后真实宫格普遍≥0.9；普通照片单格可达0.7，不宜再低）—— 兼容旧默认值，优先用 BY_SIZE 映射
    VALIDATION_MIN_CELL_UNIFORMITY = 0.90  # 同方向 cell 尺寸最小/最大比例阈值 —— 兼容旧默认值，优先用 BY_SIZE 映射
    # 按 grid_size 区分的几何校验阈值：2x2 含占位格概率高（批量生图常凑不满4格），容错更宽；
    # 3x3 保持严格。validate_grid_image 优先取 BY_SIZE，缺失时回退到上面的标量默认值。
    VALIDATION_MIN_LINE_COVERAGE_BY_SIZE = {
        SIZE_2X2: 0.60,   # 2x2 放宽：占位纯色区会拉低 coverage 均值
        SIZE_3X3: 0.75,   # 3x3 维持原值
    }
    VALIDATION_MIN_CELL_UNIFORMITY_BY_SIZE = {
        SIZE_2X2: 0.80,   # 2x2 放宽
        SIZE_3X3: 0.90,   # 3x3 维持原值
    }
