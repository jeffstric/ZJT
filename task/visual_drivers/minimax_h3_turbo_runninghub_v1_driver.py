"""
MiniMax H3 图生视频「加速版」 RunningHub v1 版本驱动实现

支持首尾帧图生视频，通过 RunningHub AI-App 接口调用「MiniMax H3 图生视频加速版」工作流。
webapp_id: 2092199541612306434

与标准版（minimax_h3_runninghub_v1_driver.py，webapp 2086436470516174849）的差异：
  1. webapp_id 不同
  2. 尾帧图片节点 nodeId 146（标准版为 145）

参数注入节点 nodeId：
  - 提示词      → nodeId 143 fieldName=text         (CR Text)
  - 首帧图片    → nodeId 114 fieldName=image        (LoadImage)
  - 尾帧图片    → nodeId 146 fieldName=image        (LoadImage，始终传，空则留空)
...
class MinimaxH3TurboRunninghubV1Driver(BaseVideoDriver):
    """
    MiniMax H3 图生视频「加速版」 RunningHub v1 版本驱动
    支持首尾帧图生视频
    """

    def __init__(self):
        super().__init__(driver_name="minimax_h3_turbo_runninghub_v1", driver_type=TaskTypeId.MINIMAX_H3_IMAGE_TO_VIDEO)
        ...
        self._webapp_id = "2092199541612306434"  # MiniMax H3 图生视频加速版 webapp ID
    ...
    async def build_create_request(self, ai_tool) -> Dict[str, Any]:
        """
        构建创建 MiniMax H3 加速版任务的完整请求参数

        支持首尾帧模式（节点映射）：
        - 提示词 → nodeId 143 (text，CR Text)
        - 首帧图片（必需）→ nodeId 114 (image，LoadImage)
        - 尾帧图片（可选）→ nodeId 146 (image，LoadImage，始终传，空则留空)
        ...
        self.logger.info(
            f"MiniMax H3 加速版驱动图片模式: {img_mode}, 首帧: {first_frame}, 尾帧: {last_frame}, 参考图: {len(reference_images)}张"
        )
        ...
        if img_mode == ImageMode.FIRST_LAST_FRAME:
            ...
            if reference_images:
                ...
                if len(reference_images) > 2:
                    self.logger.warning(f"MiniMax H3 加速版最多支持首尾帧2张图，已忽略多余的参考图")
        elif img_mode == ImageMode.FIRST_LAST_WITH_REF:
            first_image_path = first_frame
            last_image_path = last_frame
            if reference_images:
                self.logger.warning(f"MiniMax H3 加速版不支持首尾帧+参考图模式，已忽略参考图")

        if not first_image_path:
            raise ValueError("MiniMax H3 加速版任务需要至少1张首帧图片")
        ...
        # - nodeId 146 image          尾帧 LoadImage（始终传，无尾帧时 fieldValue 留空，
        ...
        self.logger.info(
            f"MiniMax H3 加速版请求参数: duration={duration}s, ratio={ratio}(aspect_ratio={aspect_ratio_value}), "
    ...
    def build_check_query(self, project_id: str) -> Dict[str, Any]:
        """
        构建查询 MiniMax H3 加速版任务状态的完整请求参数
    ...
    async def submit_task(self, ai_tool) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(
                f"Submitting MiniMax H3 加速版 task: prompt='{ai_tool.prompt[:50] if ai_tool.prompt else ''}...', "
            ...
            try:
                ...
                # 网络异常，允许重试
                self.logger.warning(f"Network error during MiniMax H3 加速版 task submission: {str(network_error)}")
                return {
                    "success": False,
                    "error": "网络连接异常，请稍后重试",
                    "error_type": "USER",
            ...
            self.logger.info(f"MiniMax H3 加速版 API response: {result}")
            ...
            if not is_valid:
                ...
                self._send_alert(
                    ...
                    message=f"MiniMax H3 加速版 submit_task 响应格式错误: {validation_error}",
                    context={
                        "api": "create_minimax_h3_turbo_image_to_video",
                        "response": result,
            ...
            if error_code or error_message:
                ...
                if is_upstream_congested_error(error_code):
                    self.logger.warning(
                        f"MiniMax H3 加速版 upstream congested, will auto-retry: "
                        f"errorCode={error_code}, errorMessage={error_message}"
                    )
                    return self._build_upstream_congested_result()
                self.logger.warning(
                    f"MiniMax H3 加速版 API returned error: errorCode={error_code}, errorMessage={error_message}"
                )
                return {
                    "success": False,
            ...
            if not task_id:
                return {
                    ...
                    "error": "服务异常，请联系技术支持",
                    "error_type": "SYSTEM",
                    "error_detail": "MiniMax H3 加速版 API未返回任务ID",
                    "retry": False
                }
            ...
            # 非网络异常，发送报警，不重试
            self.logger.error(f"Unexpected exception in MiniMax H3 加速版 submit_task: {str(e)}")
            self.logger.error(traceback.format_exc())

            self._send_alert(
                alert_type="UNEXPECTED_EXCEPTION",
                message=f"MiniMax H3 加速版 submit_task 发生未预期异常: {str(e)}",
                context={
                    "exception": str(e),
    ...
    def check_status(self, project_id: str) -> Dict[str, Any]:
        ...
        """
        try:
            self.logger.info(f"Checking MiniMax H3 加速版 task status: project_id={project_id}")
            ...
            try:
                ...
                # 网络异常，允许重试
                self.logger.warning(f"Network error during MiniMax H3 加速版 status check: {str(network_error)}")
                return {
                    "status": "RUNNING",
            ...
            if not isinstance(status_result, dict) or "code" not in status_result:
                self._send_alert(
                    alert_type="INVALID_RESPONSE_FORMAT",
                    message="MiniMax H3 加速版 status API 响应格式异常",
                    context={
                        "api": "check_status",
            ...
            # 非网络异常，发送报警
            self.logger.error(f"Unexpected exception in MiniMax H3 加速版 check_status: {str(e)}")
            self.logger.error(traceback.format_exc())

            self._send_alert(
                alert_type="UNEXPECTED_EXCEPTION",
                message=f"MiniMax H3 加速版 check_status 发生未预期异常: {str(e)}",
                context={
                    "exception": str(e),
                    "traceback": traceback.format_exc(),
                    "project_id": project_id
