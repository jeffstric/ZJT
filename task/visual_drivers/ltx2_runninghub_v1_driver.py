...
class Ltx2RunninghubV1Driver(BaseVideoDriver):
    ...
    async def build_create_request(self, ai_tool) -> Dict[str, Any]:
        ...
        self.logger.info(f"LTX2 驱动图片模式: {img_mode}, 首帧: {first_frame}, 尾帧: {last_frame}, 参考图: {len(reference_images)}张")
        ...
        try:
            ...
            if error_code or error_message:
                ...
                if is_upstream_congested_error(error_code):
                    self.logger.warning(f"LTX2 upstream congested, will auto-retry: errorCode={error_code}, errorMessage={error_message}")
