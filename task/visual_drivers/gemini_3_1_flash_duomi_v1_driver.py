"""
Gemini 3.1 Flash 多米供应商 v1 版本驱动实现
"""
...
class Gemini31FlashDuomiV1Driver(BaseVideoDriver):
    """
    Gemini 3.1 Flash 多米供应商 v1 版本驱动
    支持图片编辑
    """
    
    def __init__(self):
        super().__init__(driver_name="gemini_3_1_flash_duomi_v1", driver_type=17)
    ...
    def build_create_request(self, ai_tool) -> Dict[str, Any]:
        """
        构建创建 Gemini 3.1 Flash 任务的完整请求参数
    ...
    def build_check_query(self, project_id: str) -> Dict[str, Any]:
        """
        构建查询 Gemini 3.1 Flash 任务状态的完整请求参数
    ...
    def submit_task(self, ai_tool) -> Dict[str, Any]:
        ...
        """
        try:
            self.logger.info(f"Submitting Gemini 3.1 Flash task: prompt='{ai_tool.prompt[:50]}...', ratio={ai_tool.ratio}, size={ai_tool.image_size}")
            ...
            try:
                ...
                # 网络异常，允许重试
                self.logger.warning(f"Network error during Gemini 3.1 Flash task submission: {str(network_error)}")
                return {
                    "success": False,
                    "error": "网络连接异常，请稍后重试",
                    "error_type": "USER",
            ...
            self.logger.info(f"Gemini 3.1 Flash API response: {result}")
            ...
            if not is_valid:
                ...
                self._send_alert(
                    ...
                    message=f"Gemini 3.1 Flash submit_task 响应格式错误: {validation_error}",
                    context={
                        "api": "create_ai_image",
                        "response": result,
            ...
            if result.get("code") != 200:
                ...
                self.logger.warning(f"Gemini 3.1 Flash API returned error: code={result.get('code')}, msg={error_msg}")
                return {
                    "success": False,
            ...
            if not task_id:
                return {
                    ...
                    "error": "服务异常，请联系技术支持",
                    "error_type": "SYSTEM",
                    "error_detail": "Gemini 3.1 Flash API未返回任务ID",
                    "retry": False
                }
            ...
            # 非网络异常，发送报警，不重试
            self.logger.error(f"Unexpected exception in Gemini 3.1 Flash submit_task: {str(e)}")
            self.logger.error(traceback.format_exc())
            
            self._send_alert(
                alert_type="UNEXPECTED_EXCEPTION",
                message=f"Gemini 3.1 Flash submit_task 发生未预期异常: {str(e)}",
                context={
                    "exception": str(e),
    ...
    def check_status(self, project_id: str) -> Dict[str, Any]:
        ...
        """
        try:
            self.logger.info(f"Checking Gemini 3.1 Flash task status: project_id={project_id}")
            ...
            try:
                ...
                # 网络异常，允许重试
                self.logger.warning(f"Network error during Gemini 3.1 Flash status check: {str(network_error)}")
                return {
                    "status": "RUNNING",
            ...
            self.logger.info(f"Gemini 3.1 Flash status API response: {result}")
            ...
            if not is_valid:
                ...
                self._send_alert(
                    alert_type="INVALID_RESPONSE_FORMAT",
                    message=f"Gemini 3.1 Flash check_status 响应格式错误: {validation_error}",
                    context={
            ...
            if result.get("code") != 0:
                error_msg = result.get("msg", "未知错误")
                self.logger.warning(f"Gemini 3.1 Flash status API returned error: code={result.get('code')}, msg={error_msg}")
                return {
                    "status": "FAILED",
            ...
            # 非网络异常，发送报警
            self.logger.error(f"Unexpected exception in Gemini 3.1 Flash check_status: {str(e)}")
            self.logger.error(traceback.format_exc())
            
            self._send_alert(
                alert_type="UNEXPECTED_EXCEPTION",
                message=f"Gemini 3.1 Flash check_status 发生未预期异常: {str(e)}",
                context={
                    "exception": str(e),
                    "traceback": traceback.format_exc(),
                    "project_id": project_id
