...
from runninghub_request import create_ltx2_image_to_video, get_task_status
...
class Ltx2RunninghubV1Driver(BaseVideoDriver):
    ...
    def submit_task(self, ai_tool) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get("code") != 0:
                ...
                self.logger.warning(f"LTX2 API returned error: code={result.get('code')}, msg={error_msg}")
                return {
                    "success": False,
    ...
    def check_status(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"LTX2 status API response: {result}")
            ...
            if not is_valid:
                ...
                self._send_alert(
                    alert_type="INVALID_RESPONSE_FORMAT",
                    message=f"LTX2 check_status 响应格式错误: {validation_error}",
                    context={
                        "api": "get_task_status",
                        "response": result,
            ...
            if result.get("code") != 0:
                error_msg = result.get("msg", "未知错误")
                self.logger.warning(f"LTX2 status API returned error: code={result.get('code')}, msg={error_msg}")
                return {
                    "status": "FAILED",
