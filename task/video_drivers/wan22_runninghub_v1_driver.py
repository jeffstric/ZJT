...
from runninghub_request import create_wan22_image_to_video, get_task_status
    ...
    def submit_task(self, ai_tool) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get("code") != 0:
                ...
                self.logger.warning(f"Wan22 API returned error: code={result.get('code')}, msg={error_msg}")
                return {
                    "success": False,
    ...
    def check_status(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not is_valid:
                ...
                self._send_alert(
                    alert_type="INVALID_RESPONSE_FORMAT",
                    message=f"Wan22 check_status 响应格式错误: {validation_error}",
                    context={
                        "api": "get_task_status",
                        "response": result,
            ...
            if result.get("code") != 0:
                error_msg = result.get("msg", "未知错误")
                self.logger.warning(f"Wan22 status API returned error: code={result.get('code')}, msg={error_msg}")
                return {
                    "status": "FAILED",
