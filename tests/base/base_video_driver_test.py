...
from tests.base.db_test_config import get_unit_config_value
...
class BaseVideoDriverTest(DatabaseTestCase):
    ...
    def setUp(self):
        """测试前准备：清理 runninghub_slots 确保干净的初始状态"""
        super().setUp()
        # 清理 runninghub_slots 表（确保测试开始前没有遗留的活跃槽位）
        try:
            import pymysql
            from tests.base.db_test_config import TEST_DB_CONFIG
            cleanup_conn = pymysql.connect(
            ...
            try:
                ...
                cursor.execute("UPDATE runninghub_slots SET status = 2, released_at = NOW() WHERE status = 1")
                cleanup_conn.commit()
                logger.debug("setUp: 已清理 runninghub_slots 表中的活跃槽位")
            finally:
                cleanup_conn.close()
        except Exception as e:
            logger.warning(f"setUp: 清理 runninghub_slots 失败: {e}")
    ...
    def tearDown(self):
        ...
        # 清理 runninghub_slots 表（RunningHubSlotsModel 使用独立连接，事务提交后不会被回滚）
        # 在测试连接关闭后使用独立连接清理
        try:
            import pymysql
            from tests.base.db_test_config import TEST_DB_CONFIG
            ...
            try:
                ...
                cursor.execute("UPDATE runninghub_slots SET status = 2, released_at = NOW() WHERE status = 1")
                cleanup_conn.commit()
                logger.debug("已清理 runninghub_slots 表中的活跃槽位")
            finally:
                cleanup_conn.close()
        except Exception as e:
            logger.warning(f"清理 runninghub_slots 失败: {e}")
    ...
    def create_test_ai_tool(self, ai_tool_type, **kwargs):
        ...
        return AIToolsModel.create(**default_data)
    ...
    def create_test_task(self, task_type, task_id, **kwargs):
        ...
        default_data.update(kwargs)
        return TasksModel.create(**default_data)
