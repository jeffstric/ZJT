...
def get_test_db_config():
    ...
    仅从 config_unit.yml 读取配置，不回退到其他配置文件，
    避免误读生产数据库配置导致数据被错误删除。

    配置优先级：
    1. 环境变量 (TEST_DB_HOST, TEST_DB_PORT, TEST_DB_USER, TEST_DB_PASSWORD, TEST_DB_NAME)
    2. config_unit.yml 中的 test_database 或 database 配置节
    ...
    # 仅从 config_unit.yml 读取
    unit_config_file = os.path.join(APP_DIR, get_unit_test_config_path())
    if not os.path.exists(unit_config_file):
        raise FileNotFoundError(
            f"测试配置文件不存在: {unit_config_file}\n"
            f"单元测试必须使用独立的 config_unit.yml 配置文件，"
            f"请参考 config_unit.base.yml 创建。"
        )
    ...
    try:
        ...
        raise ValueError(f"加载测试配置文件失败: {e}")

    if not db_config:
        raise ValueError(
            f"配置文件 {get_unit_test_config_path()} 中未找到 test_database 或 database 配置节"
        )
    ...
    # 安全校验：数据库名必须以 _test 或 _unittest 结尾
