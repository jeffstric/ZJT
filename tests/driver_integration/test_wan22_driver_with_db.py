...
class TestWan22RunninghubWithDB(BaseVideoDriverTest):
    ...
    def test_build_create_request(self):
        ...
        self.assertIn('1950219582398185474', req['url'])  # webapp_id
        ...
        prompt_node = next((n for n in node_info_list if n['fieldName'] == 'text'), None)
        self.assertIsNotNone(prompt_node)
        self.assertEqual(prompt_node['fieldValue'], '测试提示词')
        
        duration_node = next((n for n in node_info_list if n['fieldName'] == 'value'), None)
    ...
    def test_build_check_query(self):
        """测试构建查询状态请求参数"""
        project_id = 'wan22_test_task_123'
    ...
    def test_submit_task_success(self):
        ...
        with patch.object(self.driver, '_request') as mock_req:
            ...
            prompt_node = next((n for n in node_info_list if n['fieldName'] == 'text'), None)
            self.assertEqual(prompt_node['fieldValue'], '测试 Wan2.2 提交成功')
    ...
    def test_check_status_success(self):
        ...
        with patch.object(self.driver, '_request') as mock_req:
            ...
            self.assertEqual(first_call.kwargs['json']['taskId'], 'wan22_task_456')
            ...
            self.assertEqual(second_call.kwargs['json']['taskId'], 'wan22_task_456')
    ...
    def test_check_status_failed(self):
        ...
        with patch.object(self.driver, '_request') as mock_req:
            ...
            self.assertEqual(call_args.kwargs['method'], 'POST')
            self.assertEqual(call_args.kwargs['json']['taskId'], 'wan22_task_789')
    ...
    def test_check_status_processing(self):
        ...
        with patch.object(self.driver, '_request') as mock_req:
            ...
            self.assertIn('/task/openapi/status', call_args.kwargs['url'])
            self.assertEqual(call_args.kwargs['method'], 'POST')
            self.assertEqual(call_args.kwargs['json']['taskId'], 'wan22_task_999')
