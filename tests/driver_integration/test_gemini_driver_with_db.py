...
# Gemini 任务类型常量
GEMINI_IMAGE_TO_VIDEO_TYPE = TaskTypeId.GEMINI_2_5_FLASH_IMAGE
GEMINI_PRO_IMAGE_TO_VIDEO_TYPE = TaskTypeId.GEMINI_3_PRO_IMAGE
GEMINI_3_1_FLASH_IMAGE = TaskTypeId.GEMINI_3_1_FLASH_IMAGE
...
class TestGeminiDuomiWithDB(BaseVideoDriverTest):
    ...
    def setUp(self):
        ...
        with patch('task.visual_drivers.gemini_duomi_v1_driver.get_dynamic_config_value', side_effect=mock_get_dynamic_config_value):
    ...
    def test_build_create_request(self):
        ...
        # 验证 json 结构
        self.assertEqual(req['json']['model'], 'gemini-2.5-flash-image')
    ...
    def test_submit_task_success(self):
        ...
        with patch.object(self.driver, '_request') as mock_req:
            ...
            # 验证 json 参数
            self.assertEqual(call_args.kwargs['json']['model'], 'gemini-2.5-flash-image')
    ...
    def test_gemini_pro_build_create_request_2K_image_size(self):
        """测试构建创建任务请求参数 - Gemini Pro 模型，2K 图片尺寸"""
        task_id = self.create_test_ai_tool(
            ai_tool_type=GEMINI_PRO_IMAGE_TO_VIDEO_TYPE,
            prompt='测试 Gemini Pro 提示词',
            image_path='https://example.com/test.jpg',
            ratio='16:9',
            image_size='2K',
            status=AI_TOOL_STATUS_PENDING
        ...
        # 验证 model 为 Gemini Pro 模型
        self.assertEqual(req['json']['model'], 'gemini-3-pro-image-preview')
        self.assertEqual(req['json']['image_size'], '2K')

    def test_gemini_pro_build_create_request_3_1_flash_image(self):
        """测试构建创建任务请求参数 - Gemini 3.1 Flash 模型"""
        task_id = self.create_test_ai_tool(
            ai_tool_type=GEMINI_3_1_FLASH_IMAGE,
            prompt='测试 Gemini 3.1 Flash 提示词',
            image_path='https://example.com/test.jpg',
            ratio='9:16',
            status=AI_TOOL_STATUS_PENDING
        )
        ...
        # 验证 model 为 Gemini 3.1 Flash 模型
        self.assertEqual(req['json']['model'], 'gemini-3.1-flash-image-preview')

    def test_gemini_pro_submit_task_success(self):
        """测试提交任务 - 成功 - Gemini Pro 模型"""
        task_id = self.create_test_ai_tool(
            ai_tool_type=GEMINI_PRO_IMAGE_TO_VIDEO_TYPE,
    ...
    def test_gemini_pro_submit_task_without_image_size(self):
        """测试提交任务 - image_size 为空时使用默认值 1K - Gemini Pro 模型"""
        task_id = self.create_test_ai_tool(
            ai_tool_type=GEMINI_PRO_IMAGE_TO_VIDEO_TYPE,
            prompt='测试无 image_size',
            image_path='https://example.com/test.jpg',
            ratio='16:9',
            status=AI_TOOL_STATUS_PROCESSING
        )
    ...
    def test_gemini_pro_submit_task_with_multiple_images(self):
        """测试提交任务 - 多张图片 - Gemini Pro 模型"""
        task_id = self.create_test_ai_tool(
            ai_tool_type=GEMINI_PRO_IMAGE_TO_VIDEO_TYPE,
            prompt='测试 Gemini Pro 多张图片',
            image_path='https://example.com/img1.jpg,https://example.com/img2.jpg,https://example.com/img3.jpg',
            ratio='16:9',
            image_size='2K',
            status=AI_TOOL_STATUS_PROCESSING
        ...
        with patch.object(self.driver, '_request') as mock_req:
            mock_req.return_value = {
                "code": 200,
                "msg": "success",
                "data": {"task_id": "gemini_pro_task_multi"}
            }
    ...
    def test_gemini_pro_check_status_success(self):
        """测试检查状态 - 成功 - Gemini Pro 模型"""
        task_id = self.create_test_ai_tool(
            ai_tool_type=GEMINI_PRO_IMAGE_TO_VIDEO_TYPE,
            prompt='测试 Gemini Pro 状态检查成功',
            image_path='https://example.com/test.jpg',
            status=AI_TOOL_STATUS_PROCESSING,
            project_id='gemini_pro_task_456'
        )

        # Mock _request 返回原始 API 响应（Pro 模型返回图片而非视频）
        with patch.object(self.driver, '_request') as mock_req:
            mock_req.return_value = {
                "code": 200,
                "data": {
