...
背景：体验包 9.9（package_id=1）曾被当作首充福利包（已首充用户不可见/不可购），
现常规化为任意时刻可充；首充福利由新套餐 package_id=5（0.1 元 / 99 算力）承载。

覆盖：
- 未首充用户：可见全部套餐（含 0.1 元首充福利包与 9.9 体验包）
- 已首充用户：仅过滤首充福利包（package_id=5），9.9 体验包（package_id=1）仍可见（核心回归点）
...
def test_recharged_user_still_sees_9_9_package(client, monkeypatch):
    """核心回归：已首充用户看不到 0.1 元福利包，但 9.9 体验包（package_id=1）必须可见"""
