...
_HAS_ENTERPRISE_CONTRACT = (
    importlib.util.find_spec("enterprise") is not None
    and importlib.util.find_spec("enterprise.services.script_split_quality.contract")
    is not None
)

if _HAS_ENTERPRISE_CONTRACT:
...
@unittest.skipUnless(
    _HAS_ENTERPRISE_CONTRACT,
    "enterprise.services.script_split_quality.contract not available",
)
class TestScriptSplitQualityPlanContract(unittest.TestCase):
    def test_mode_none_allows_discontinuous_boundary(self):
        ...
        self.assertEqual(compiled["segments"][1]["spatial_dependency"]["mode"], "none")
        # character_key 已编译为 character_id
        self.assertEqual(
            compiled["segments"][1]["continuity_in"]["characters"][0]["character_id"],
            "char_001",
        )
        self.assertEqual(
    ...
    def test_inherit_requires_continuity_in_match_upstream_out(self):
        ...
        with self.assertRaisesRegex(
            QualityPlanError, "continuity_out does not match seg_0002"
        ):
            compile_quality_plan(plan, _anchors_for("block_0001", "block_0002"))

    def test_inherit_passes_when_in_equals_upstream_out(self):
        lobby = ("character:lin", "su_lobby", "cont_lobby", "slot_desk")
        plan = {
            "schema_version": 2,
            "entities": _base_entities(),
        ...
        compiled = compile_quality_plan(plan, _anchors_for("block_0001", "block_0002"))
        self.assertEqual(
            compiled["segments"][1]["spatial_dependency"]["from_segment_id"],
            "seg_0001",
        )

    def test_inherit_matches_from_segment_not_list_predecessor(self):
        ...
        self.assertEqual(
            compiled["segments"][2]["spatial_dependency"]["from_segment_id"],
            "seg_0001",
        )
    ...
    def test_compile_l0_accepts_db_matched_location_and_child(self):
        ...
        self.assertEqual(by_id["loc_001"]["location_db_id"], 565)
        self.assertEqual(by_id["loc_002"]["parent_id"], "loc_001")
        self.assertIsNone(by_id["loc_002"]["location_db_id"])
