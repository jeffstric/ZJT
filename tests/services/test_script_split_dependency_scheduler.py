...
def test_failed_segment_is_ready_for_retry():
    ...
    assert [s["segment_id"] for s in ready] == ["seg_0002"]

    ready2, waiting, blocked = classify_segments(plan, all_segments)
    assert [s["segment_id"] for s in ready2] == ["seg_0002"]
    assert [w["segment"]["segment_id"] for w in waiting] == ["seg_0003"]
    assert waiting[0]["from_status"] == "failed"
    assert blocked == []


def test_upstream_failed_is_waiting_not_blocked():
    ...
    assert waiting[0]["segment"].segment_id == "seg_0003"
    assert waiting[0]["from_segment_id"] == "seg_0002"
    assert waiting[0]["from_status"] == "failed"
    assert blocked == []


def test_pending_with_completed_upstream_is_ready():
    plan = _plan_chain()
    all_segments = [
...
def test_completed_and_generating_never_ready():
    plan = _plan_chain()
    all_segments = [
...
def test_missing_upstream_is_blocked():
    plan = {
        "segments": [
            {
                "segment_id": "seg_0002",
    ...
    assert ready == []
    assert waiting == []
    assert [s["segment_id"] for s in blocked] == ["seg_0002"]


def test_completed_upstream_without_parsed_is_blocked():
    plan = _plan_chain()
    all_segments = [
...
def test_select_ready_respects_limit_and_order():
    plan = {
        "segments": [
    ...
    assert [s["segment_id"] for s in ready] == ["seg_a", "seg_b"]
