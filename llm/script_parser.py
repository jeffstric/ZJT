...
6. 分镜要包含镜头类型、运动方式、对话、动作等详细信息
7. opening_frame_description是最关键字段,用于AI生成首帧图像,必须非常详细描述镜头起始画面（包括人物位置、姿态、表情、场景布局、光线效果、构图信息等）
8. 确保所有ID引用关系正确（如shot中的location_id和character_id要对应）
9. 只输出纯JSON内容,不要添加```json```标记或任何解释性文字
10. **【重要】在shot节点的所有文本字段中,只要涉及角色名称,必须用【【角色名】】格式包裹,便于后续匹配角色库**
