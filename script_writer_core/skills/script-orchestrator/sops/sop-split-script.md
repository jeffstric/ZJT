...
1. **确认拆分需求**
   - **⚠️ 必须先识别长文本剧本文件名（关键）**：
     - 检查用户消息中是否包含长文本系统提示（形如"用户输入的内容超过5000字，已自动保存完整内容。- 文件名：HH:MM:SS.txt"）
     - 如果存在该提示，**必须记录下文件名**（如 `14:57:23.txt`），后续调用 novel-episode-splitter 时必须透传该文件名
     - 这是因为：当剧本超过 5000 字时，完整内容已存为文件，PM 当前看到的只是截断后的预览，拆分专家必须通过 `get_long_user_input(name="文件名")` 才能读到完整剧本
     - 如果不透传文件名，拆分专家会因无法访问完整内容而反馈"没有提供导入剧本"
   - **必须向用户确认**：
     - 先向用户展示已收集的信息摘要，然后调用：
     ```
     ask_user(
...
2. **调用 novel-episode-splitter**
   - **⚠️ 长文本剧本透传规则（必须遵守）**：
     - 如果步骤1识别到长文本文件名（剧本 >5000 字），task_description 中**必须包含**：文件名 + 明确指示拆分专家调用 `get_long_user_input(name="文件名")` 读取完整内容
     - **禁止**直接把截断后的预览内容或字面占位符 `[剧本内容]` 当作完整剧本传给拆分专家
     - 系统（PM Agent）已会自动把文件名注入拆分专家的上下文，但你仍需在 task_description 中显式写明，确保万无一失
   - **任务描述（长文本模式，剧本 >5000 字时使用）**：
     ```
     call_agent(
       AgentName: "novel-episode-splitter",
       task_description: "请将用户提供的小说/剧本按照每集[X]分钟的时长进行拆分，剧本标题为《[标题]》。

       【重要】剧本内容超过5000字，完整内容已保存为文件，请立即调用 get_long_user_input(name=\"[文件名]\") 读取完整剧本后再开始拆分。不要使用上下文中截断的预览内容。
   ...
   - **输入**：
     - 剧本标题
     - 每集时长（分钟）
     - 长文本文件名（剧本 >5000 字时，由步骤1识别）或完整的小说/剧本内容（≤5000字）
   - **输出**：
     - 拆分后的剧本文件（每集一个JSON文件）
     - 拆分统计报告
...
5.5. **调用 location-prop-image-designer（为所有缺图场景道具生成形象）**
   ...
   - **⚠️ 执行要求**：步骤5.3展示完成后，**必须在同一轮对话中立即调用**，不要等待用户回复
