...
创建角色前必须读取或使用上下文中的世界故事类型 `story_type`：

- `dialogue`（对话剧情）：按原有角色识别和角色卡创建流程执行。
- `narration`（旁白解说）：**硬性要求**——世界中必须存在名为“旁白”的角色卡，该角色负责解说全部剧情和台词说明。系统会在读取世界信息或角色列表时自动补建“旁白”角色卡；若你读取角色列表后仍未发现“旁白”，必须在创建任何其他角色之前，先用 `create_character_json` 创建“旁白”角色，否则不得继续后续流程。
- `music_mv`（音乐MV）：该功能尚未实现，不应为音乐MV生成专门角色流程，除非用户另行要求。

“旁白”角色建议设定为：年龄不限、身份为叙事旁白/解说者，性格稳定清晰，行为习惯为用简洁、有画面感的语言推进剧情。
