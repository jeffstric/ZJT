...
输出要求：
1. 必须严格按照指定的JSON格式输出
2. 分镜组默认每个15秒,可根据剧情需要调整；若用户提示中给出「本段总时长预算」，必须遵守该预算：本段所有镜头 duration 总和必须落在预算的±15%区间内——不得明显低于下限，也不得超过上限。增加总时长的正确方式是拆出更多镜头（把动作/情绪/剧情节拍拆细，增加反应、细节特写、过渡、氛围空镜与视角切换），而不是拉长单个镜头：单镜头时长保持在3~8秒的正常叙事节奏，禁止为凑预算拉长到10秒以上；只有按正常节奏拆分后仍难达下限时，才允许适当放慢关键动作与对白节奏
3. 人物信息要完整,包括角色定位和描述
...
19. **【分镜呈现类型 presentation】** 每个 shot 必须输出 presentation 字段，取值仅限：
    - `"digital_human"`：对口型/数字人镜头——必须同时满足：`characters_present` **恰好只有 1 个画内人物**、`dialogue` **恰好只有 1 个说话角色**、且说话角色就是该画内人物；内容以对白表演为主，适合近景/特写固定镜头下的口型视频。
    - `"video"`：普通 AI 视频镜头——无对白、旁白/画外音、没有人物、多人同框、多人交替说话，或虽有单人对白但核心是动作/追逐/打斗/复杂调度。
    - **硬性约束**：只要 `characters_present` 不是恰好 1 个（0 个或 2 个及以上），presentation **必须**为 `"video"`；即使多人画面中只有一个人开口，也不可标为 digital_human。
    - **硬性约束**：dialogue 中出现 2 个及以上不同说话角色，或唯一说话角色不在 `characters_present`（画外音）时，presentation **必须**为 `"video"`。
    - **非人物画面**：昆虫、普通动物、物体、风景等无人画面一律为 `"video"`。例如“蚊子微距特写 + 人物画外音”必须为 `"video"`，不可因存在台词而标为 digital_human。
